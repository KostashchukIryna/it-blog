...
На основі аудиту (завдання 1) виправлено 3 проблеми у проєкті:

| № | Проблема | Тип | Що зроблено | URL де виправлено |
|:---:|:---|:---|:---|:---|
| 1 | Використання неінформативного анкору "Детальніше тут" | анкор (generic) | Замінено загальний текст посилання на динамічний описовий анкор "Більше статей про [назва категорії]". | Шаблон статті (`/[category]/[slug]`) |
| 2 | Відсутність контекстного посилання на сторінку авторів | nav-only | Текстове поле з ім'ям автора обгорнуто в компонент `<Link>`, що створює пряме посилання на `/authors`. | Шаблон статті (`/[category]/[slug]`) |
| 3 | Використання JavaScript (`onClick`) для переходу замість тегів `<a>` | інше (JS-посилання) | Блок картки у секції "Читайте також" переписано: `<div onClick={...}>` замінено на SEO-дружній компонент `<Link>`. | Шаблон статті (`/[category]/[slug]`) |
---
### 1. Використання неінформативного анкору "Детальніше тут" замінено на загальний текст посилання на динамічний описовий анкор "Більше статей про [назва категорії]"

#### До
![Скріншот ДО](before1.jpg)

#### Після
![Скріншот ПІСЛЯ](after1.jpg)

---

### 2. Відсутність контекстного посилання на сторінку авторів. Змінено: текстове поле з ім'ям автора обгорнуто в компонент `<Link>`, що створює пряме посилання на `/authors`.

#### До
![Скріншот ДО](before2.jpg)

#### Після
![Скріншот ПІСЛЯ](after2.jpg)

---
### 3. Використання JavaScript (`onClick`) для переходу замість тегів `<a>` було змінено на компонент `<Link>`.

#### До
![Скріншот ДО](before3.jpg)

#### Після
![Скріншот ПІСЛЯ](after3.jpg)
