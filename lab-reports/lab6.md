...
| https://it-blog-news.pp.ua/career/why-clean-code-matters | Mobile | 96 | 2.7s | N/A | 0 | 60ms | 0.9s | Good |
| https://it-blog-news.pp.ua/career/why-clean-code-matters | Desktop | 100 | 0.5s | N/A | 0.003 | 0s | 0.3s | Good |
...
| https://it-blog-news.pp.ua/career/why-clean-code-matters | Покращте показ зображень | LCP | Зменшення часу завантаження зображень може прискорити візуалізацію сторінки й покращити показник LCP | High |
| https://it-blog-news.pp.ua/career/why-clean-code-matters | Застарілі функції JavaScript | LCP | Щоб оптимізувати показник LCP, зробіть зображення LCP видимим у HTML-коді відразу й не використовуйте відкладене завантаження | Medium |
...
### Опис впроваджених змін

Для покращення показників швидкодії та стабільності сайту було реалізовано 4 ключові технічні правки:

### Оптимізація LCP
* **Що змінено:** У файлі `app/[...slug]/page.tsx` до головного зображення статті додано атрибут `priority={true}`.
* **Технічний ефект:** Це дає вказівку браузеру завантажувати це зображення з найвищим пріоритетом, не чекаючи завантаження скриптів, що суттєво прискорює появу найбільшого контенту на екрані.

### Оптимізація CLS — Шрифти
* **Що змінено:** У файлі `layout.tsx` для шрифту **Raleway** додано параметр `display: "swap"`.
* **Технічний ефект:** Це усуває проблему FOIT (Flash of Invisible Text). Браузер миттєво показує текст системним шрифтом, доки завантажується основний, що запобігає неочікуваним зсувам текстових блоків.

### Оптимізація CLS — Зображення
* **Що змінено:** Для всіх карток статей впроваджено використання контейнерів із фіксованим співвідношенням сторін (`aspect-video`) та заміну тегів `<img>` на компонент `next/image` з властивістю `fill`.
* **Технічний ефект:** Браузер заздалегідь знає точні розміри блоку для картинки та резервує під неї місце. Контент, що знаходиться нижче, більше не "стрибає" після фактичного завантаження файлу зображення.

### Серверна оптимізація та кешування
* **Що змінено:** У `Navbar` налаштовано отримання категорій через API з використанням стратегії автоматичної генерації на стороні сервера.
* **Технічний ефект:** Зменшення обсягу клієнтського коду та пришвидшення відповіді сервера за рахунок використання вбудованих механізмів оптимізації Next.js для доставки ресурсів.

---

### Результати вимірювання

Вимірювання проводилося за допомогою інструменту **Google PageSpeed Insights**.

| Метрика | Було | Стало | Delta | Статус |
| :--- | :--- | :--- | :--- | :--- |
| **LCP** (mobile) | 2.7s | 1.4s | **-1.3s** | Покращено |
| **CLS** | 0.003 | 0.003 | **0** | Без змін |
| **LCP** (desktop) | 0.8s | 0.4s | **-0.4s** | Покращено |
| **Performance Score** | 96 | 100 | **+4** | Покращено |
...
14. **Якщо команда має лише 1 місяць і обмежений ресурс, що дасть більший ефект: технічні виправлення чи зовнішня оптимізація? Обґрунтуйте для вашого проєкту.**
    *Відповідь:* **Технічні виправлення.**
