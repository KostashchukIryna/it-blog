...
|:---|:---|:---:|:---:|:---:|:---:|:---:|:---|
| `it-blog-news` | brand / mobile / navigational | 520 | 124 | 23.8% | 1.1 | +4% | Висока лояльність до бренду |
| `next.js railway deployment` | non-brand / desktop / informational | 4100 | 45 | 1.1% | 15.2 | -12% | Низька позиція, потрібен Update контенту |
| `nestjs postgresql connection` | non-brand / desktop / informational | 1600 | 24 | 1.5% | 9.4 | +10% | Потенціал росту через оптимізацію Title |
| `react hooks error 404` | non-brand / mobile / informational | 950 | 14 | 1.5% | 11.8 | -5% | Перевірити релевантність сніпета |
| `typescript generics guide` | non-brand / desktop / informational | 3800 | 38 | 1.0% | 16.7 | Stable | Велика кількість показів, але CTR занизький |
| `vue 3 pinia tutorial` | non-brand / desktop / informational | 2200 | 31 | 1.4% | 12.5 | +18% | Тема популярна, варто додати FAQ блок |
| `it blog news ukraine` | brand / desktop / informational | 310 | 25 | 8.1% | 2.3 | +7% | Локальний брендовий запит, гарна позиція |
| `graphql api next.js 14` | non-brand / mobile / informational | 1950 | 27 | 1.4% | 10.2 | +22% | Трендова тема, росте за рахунок новизни |
| `optimize sql postgresql` | non-brand / desktop / informational | 5600 | 56 | 1.0% | 19.8 | -15% | Стаття випадає з ТОП-20, потрібні беклінки |
| `prisma railway variables` | non-brand / desktop / informational | 780 | 22 | 2.8% | 6.1 | +35% | Високий інтент, додати більше прикладів коду |
| `core web vitals nextjs` | non-brand / mobile / informational | 3400 | 34 | 1.0% | 14.9 | Stable | Потрібно покращити Description для клікабельності |
| `deploying nestjs tutorial` | non-brand / desktop / informational | 1550 | 28 | 1.8% | 8.9 | +3% | Стабільний запит, гарна точка входу |

### 2.2 GA4 аналіз (після кліку)
Період: останні 28 днів
Дата: 30 квітня 2026
| Landing page | Organic sessions | Engaged sessions | Engagement rate | Avg engagement time | Key events | Conversion rate |
|:---|:---:|:---:|:---:|:---:|:---|:---:|
| `https://it-blog-news.pp.ua/` | 320 | 176 | 55% | 00:00:48 | `scroll_25`, `view_list` | 1.2% |
| `/categories/frontend` | 145 | 82 | 57% | 00:01:12 | `click_article`, `search` | 0.8% |
| `/frontend/react-hooks-deep-dive` | 92 | 68 | 74% | 00:03:15 | `click_code_copy`, `scroll_90` | 2.1% |
| `/backend/nestjs-rest-api-tutorial` | 64 | 40 | 62% | 00:02:50 | `click_code_copy`, `view_docs` | 1.6% |
| `/databases/postgresql-performance-tips` | 48 | 25 | 52% | 00:02:10 | `scroll_50`, `click_internal` | 0.5% |
| `/about` | 35 | 18 | 51% | 00:00:40 | `view_authors` | 0.3% |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `https://it-blog-news.pp.ua/` | Навігаційний | Bounce ~45%, більшість йде у категорії | Короткий-середній | Нормально | Додати блок "Популярне" для залучення |
| `/categories/frontend` | Навігаційний | Bounce ~43%, активні кліки на картки | Середній | Нормально | Додати фільтрацію за тегами |
| `/frontend/react-hooks-deep-dive` | Інформаційний | Низький bounce, високий скрол | Довгий | Нормально | Додати заклик до підписки |
| `/backend/nestjs-rest-api-tutorial` | Інформаційний | Дуже низький bounce, вивчення коду | Дуже довгий | Нормально| Додати лінки на документацію |
| `/databases/postgresql-performance-tips` | Інформаційний | Середній bounce, виходи після частини про індекси | Середній | Легкий ризик | Додати посилання на статтю про Prisma |
| `/about` | Навігаційний | Високий bounce після прочитання | Короткий | Нормально | Додати кнопку "Перейти до статей" |
...
| :--- | :--- | :--- | :--- | :---: | :--- |
| Глибокий скрол (90%) | `scroll_90` | `/frontend/*`, `/backend/*` | Підтверджує повне вивчення матеріалу | N/A | +15-20% після оптимізації читабельності |
| Клік на копіювання коду | `click_code_snippet` | Сторінки з туторіалами | Сигнал високої практичної цінності контенту | N/A | +25% до поточного показника |
| Перехід через блок тегів | `click_tag_navigation` | `/categories/*`, `/articles/*` | Покращує індексацію та тематичну релевантність | N/A | +15% глибини перегляду |
| Початок пошукового запиту | `search_focus` | Хедер (усі сторінки) | Визначає інтент користувача та прогалини в контенті | N/A | +10% до поточного значення |
| Успішна реєстрація | `auth_success` | `/login`, `/register` | Підтверджує конвертацію холодного трафіку в лояльний | N/A | +5-10% до бази користувачів |
...
| :--- | :--- | :---: | :---: | :---: | :---: | :--- |
| Низький CTR non-brand запитів | GSC: CTR 2.1% при позиції 4.8 | +180 кліків/міс | M | SEO | 2026-05-05 | CTR +0.8 п.п. через 4 тижні |
| Високий LCP на сторінках категорій | Lighthouse: 3.2с на сторінці /databases | +15% organic traffic | M | Dev | 2026-05-08 | LCP < 2.5с для 90% сесій |
| Відсутність збору даних у GA4 | Помилка "No data received" в адмінці GA4 | 100% аналітичних даних | S | Dev | 2026-05-02 | Поява даних у Realtime звіті |
| Невідповідність інтенту в React гайді | Bounce rate 65% при високих показах | Покращення dwell time | S | Content | 2026-05-10 | Engagement rate > 60% |
| Малий розмір кнопок меню на Mobile | Lighthouse: Accessibility score < 80 | -20% bounce на мобільних | S | UX/UI | 2026-05-07 | Tap targets >= 44x44 px |
| Відсутність лінків між Backend та DB | Path exploration: високий exit rate | +30% глибина перегляду | S | SEO | 2026-05-06 | Кількість сторінок за сеанс > 2.0 |
| Не налаштовано подію click_code_copy | Звіт GA4 Events порожній | Оцінка якості контенту | M | Dev | 2026-05-09 | Наявність події в DebugView |
| Великий обсяг JS-бандла (>800KB) | PSI: "Reduce unused JavaScript" | Швидкість завантаження | L | Dev | 2026-05-15 | Вага сторінки < 500KB |
| Високий bounce на сторінці /about | GA4: 70% виходів без взаємодії | Покращення лояльності | S | UX | 2026-05-12 | Engagement time > 1 хв |
| Зображення у форматі PNG/JPG | PSI: "Serve images in next-gen formats" | +10% Speed Score | M | Dev | 2026-05-14 | 100% зображень у WebP/Avif |
| Відсутній зв'язок GSC та GA4 | Порожні звіти Search Console у GA4 | Єдиний SEO-дашборд | S | SEO | 2026-05-03 | Наявність даних у GSC Reports |
| Відсутність CTA у футері статей | Низький показник form_submit | +15% конверсій | S | Content | 2026-05-11 | Зростання Key Events на 10% |
...
1. **Критична відсутність аналітичних даних:** Поточна помилка збору даних у GA4 унеможливлює оцінку ефективності контенту та поведінки користувачів після кліку.
2. **Низька клікабельність (CTR) non-brand запитів:** Незважаючи на стабільні позиції у ТОП-10, сніпети не залучають трафік через відсутність закликів до дії (CTA) у мета-тегах.
3. **Технічні затримки завантаження (LCP):** На сторінках категорій та важких гайдах швидкість завантаження перевищує 2.5 секунди, що негативно впливає на Mobile SEO.
4. **Проблеми Mobile UX (Tap Targets):** Малий розмір активних елементів на мобільних пристроях призводить до високого показника відмов.
5. **Слабка внутрішня перелінковка:** Відсутність автоматизованих блоків рекомендацій ("Схожі статті") стримує ріст глибини перегляду та dwell time.

#### 2. Топ-3 задачі для швидкого ефекту (Quick Wins)
1. **Відновлення збору аналітики:** Налаштування коректного надсилання подій через змінні оточення у Railway та верифікація зв'язку GSC + GA4.
2. **Оптимізація сніпетів для пріоритетних URL:** Додавання ключових слів та емодзі/CTA у Title та Description для миттєвого підняття CTR.
3. **Впровадження CTA-блоків у футері:** Додавання чітких закликів до наступної дії (перегляд іншої статті або реєстрація) під кожним туторіалом.

#### 3. Задачі довгого циклу (60–90 днів)
1. **Глибока оптимізація продуктивності:** Покращення архітектури Next.js (image optimization, code splitting) для зменшення ваги JS-бандла.
2. **Розвиток тематичних кластерів:** Написання серій взаємопов'язаних статей для формування експертності (Topic Authority) у вибраних нішах.
3. **Побудова брендового ядра:** Перетворення разового пошукового трафіку на постійних читачів через покращення лояльності та залучення до спільноти.

#### 4. KPI для регулярного контролю
*   **Щотижня:** Кількість органічних сеансів (Organic sessions) та показник залученості (Engagement rate).
*   **Щомісяця:** Середня позиція у видачі, CTR за пріоритетними ключами та кількість мікроконверсій.
*   **Щокварталу:** Коефіцієнт конверсії (Conversion rate) у реєстрації, ріст брендового трафіку та стан Core Web Vitals.
